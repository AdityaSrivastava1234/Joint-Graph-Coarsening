...
def coarsen(
    ...
    for level in range(1, max_levels + 1):
        ...
        if W0 is not None:
            W0 = graph_utils.zero_diag(coarsen_matrix(W0, iC))  # coarsen and remove self-loops
            W0 = (W0 + W0.T) / 2  # this is only needed to avoid pygsp complaining for tiny errors
...
def contract_variation_linear(G, A=None, K=10, r=0.5, mode="neighborhood"):
    ...
    family.sort(key=operator.attrgetter('cost'))
